...
try
{
	...
	warning= request.getParameter("w");
}
...
String viewurl = "viewRnaResult.jsp?db=" + database + "&r=" + directionField + "&f=1&t=" + outFileNum + "&s=" + resultSize + "&fp=" + filepath+ "&c=0";
	...
	<head>
		...
		function filterData()
		{
			...
			else
			{
				...
				window.location = "viewRna.jsp?db="+ db +"&r="+ df +"&f=" + value +"&t=" + outFileNum + "&c=0&s=" + <%= resultSize %>+"&fp=" + filepath;
			}
		}
...
<!--  LOGO PANEL -------------------------------------------------------------------------------------------------------------------------->			
 ...
 <!--  TABS PANEL --------------------------------------------------------------------------------------------------------------------------->				
	...
	<div id="tabsF">
	    <ul>
			<li><a href="#tab1"><span> &nbsp;Results &nbsp;</span></a></li>
			<li><a href="<%=viewurl%>"><span> &nbsp;Plots &nbsp;</span></a></li>
...
<!--  TOOLBAR PANEL ----------------------------------------------------------------------------------------------------------------------->			
				...
				<div id="toolbarPanel">
				 ...
				 <input type="hidden" id="outFileNum" value="<%= outFileNum %>" />
				  <% if(!warning.equals("Done")){%>    <%= warning  %> <br/>
					 <br/><% } %>
				 ...
				 {
				  %>
				 <a href="viewRna.jsp?db=<%= database %>&r=<%= directionField %>&fp=<%=filepath %>&f=<%= filter %>&t=<%= outFileNum %>&c=<%= (cursor - navsize) %>&s=<%= resultSize %>">&#60;Previous</a> 
				 <%
				 }
				 if(count < resultSize)
				 {
				  %>
				  <a href="viewRna.jsp?db=<%= database %>&r=<%= directionField %>&fp=<%=filepath %>&f=<%= filter %>&t=<%= outFileNum %>&c=<%= setsize %>&s=<%= resultSize %>">Next&#62</a>
				  <%
				 }
				 System.out.println("just above the call");
				 %>
				 <br/><br/>
